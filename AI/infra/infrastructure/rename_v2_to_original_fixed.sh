...
# ============================================
# OpenSearch _v2 인덱스를 원본 이름으로 변경 (Alias 처리 포함)
# ============================================
...
for V2_INDEX in $V2_INDICES; do
    ...
    if [ "$ORIGINAL_COUNT" == "$V2_COUNT" ] && [ "$ORIGINAL_COUNT" != "null" ] && [ "$ORIGINAL_COUNT" != "0" ]; then
        echo "  ✅ 검증 성공!"
...
echo ""
echo "Alias 목록:"
curl -s "$OPENSEARCH_HOST/_cat/aliases/*_2025_*?v&h=alias,index"
