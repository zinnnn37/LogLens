...
class h {
  ...
  static patchPromise() {
      ...
      constructor(n) {
        ...
        super((a, l) => {
          n(
            ...
            r
              ? u => {
                  (u && typeof u == 'object' && (u.__traceId = r.traceId),
                    e.run(r, () => l(u)));
                }
              : l,
          );
    ...
    ((globalThis.Promise = s),
      (s.resolve = function (i) {
        return new s(n => n(i));
      }),
      (s.reject = function (i) {
  ...
  static patchTimer() {
    ...
    ((this.originalSetTimeout = globalThis.setTimeout.bind(globalThis)),
      ...
      (globalThis.setTimeout = function (e, s, ...i) {
        const n = t.current(),
          r = function (...a) {
            if (n) {
              t.stack.push(n);
...
class I {
  ...
  static toConsole(t) {
    const e = Math.max(0, h.getDepth() - 1),
      s = '|'.repeat(e),
      i = '→',
      n = this.COLORS[t.level],
      r = this.COLORS.RESET,
      a = this.COLORS.GRAY,
      l = `${n}[${t.level.padStart(this.LEVEL_WIDTH)}]${r}`,
      u = this.getDurationColor(t.executionTimeMs),
      p =
        t.executionTimeMs !== null
          ? `${u}(${t.executionTimeMs.toString().padStart(5)}ms)${r}`
          : '',
      c = t.logger || t.message || 'anonymous',
      g = t.message ? `${c}: ${a}${t.message}${r}` : c;
    return `${l} ${s}${i} ${g} ${p}`;
  }
  /**
   * Duration 기반 색상 선택
...
async function E() {
  return (
    (await (await fetch('https://api.ipify.org?format=json')).json()).ip || null
  );
}
class f {
  static instance;
  ...
  excludePatterns = [];
  constructor() {
    ((this.sensitiveKeyPatterns = [...f.DEFAULT_SENSITIVE_KEY_PATTERNS]),
      (this.sensitiveValuePatterns = [...f.DEFAULT_SENSITIVE_VALUE_PATTERNS]));
  }
  /**
   * Singleton
   */
  ...
  loadFromConfig(t) {
    (t.sensitive &&
      t.sensitive.forEach(e => {
        this.sensitiveKeyPatterns.push(new RegExp(`^${e}$`, 'i'));
      }),
      t.sensitivePatterns &&
        t.sensitivePatterns.forEach(e => {
          this.sensitiveKeyPatterns.push(new RegExp(e, 'i'));
        }),
      t.exclude &&
        t.exclude.forEach(e => {
  ...
  async loadFromFile(t) {
    try {
      if (typeof require < 'u') {
        const s = await require('fs').promises.readFile(t, 'utf-8'),
          i = JSON.parse(s);
        (this.loadFromConfig(i),
          console.log(`[DataMasker] Loaded config from ${t}`));
      }
    } catch {}
  }
  /**
  ...
  maskRecursive(t, e) {
    if (t == null) return t;
    if (typeof t == 'object' && e.has(t)) return '[Circular]';
    if (typeof t != 'object')
      return typeof t == 'string' && this.isSensitiveValue(t)
        ? this.maskValue(t)
        : t;
    ...
    for (const i in t) {
      ...
      this.shouldExclude(i)
        ? (s[i] = '<excluded>')
        : this.isSensitiveKey(i)
          ? (s[i] = '****')
          : typeof n == 'string' && this.isSensitiveValue(n)
            ? (s[i] = this.maskValue(n))
            : typeof n == 'object' && n !== null
              ? (s[i] = this.maskRecursive(n, e))
              : (s[i] = n);
    }
    return s;
  }
  ...
  maskValue(t) {
    if (/@/.test(t) && t.includes('.')) {
      const [e, s] = t.split('@');
      if (e.length > 0) return `${e[0]}***@${s}`;
    }
    if (/^\d{6}-[1-4]\d{6}$/.test(t)) return t.substring(0, 2) + '****-*******';
    if (/^\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}$/.test(t))
      return t.replace(/\d(?=[\d\s-]*\d{4}$)/g, '*');
    if (
      /^(\+?\d{1,3}[-.\s]?)?\(?\d{2,4}\)?[-.\s]?\d{3,4}[-.\s]?\d{4}$/.test(t)
    ) {
      const e = t.replace(/\D/g, '');
      if (e.length >= 8) return '*'.repeat(e.length - 4) + e.slice(-4);
    }
    return '****';
  }
  /**
...
class d {
   ...
   */
  static async init(t) {
    ((this.masker = await f.initialize()),
      t === null
        ? (this.config = {
            maxLogs: 1e3,
            autoFlush: {
      ...
      this.config.autoFlush?.enabled && this.startAutoFlush());
  }
  /**
   * 로그 추가
   */
  static addLog(t) {
    if ((this.logs.push(t), this.config.isProduction))
      t.level === 'ERROR' &&
        t.logger !== 'ErrorCapture' &&
        (console.error('[LogLens Error]', t.message),
        t.response && console.error('Context:', t.response));
    else {
  ...
  static async sendLogs(t) {
    ...
    const e = [...this.logs];
    this.logs = [];
    const s = e.map(n => this.masker.mask(n)),
      i = await E();
    try {
      ...
      if (!n.ok) throw new Error(`Failed to send logs: ${n.status}`);
      this.config.isProduction ||
        console.log(`[LogCollector] Successfully sent ${e.length} logs`);
    } catch (n) {
      (console.error('[LogCollector] Failed to send logs:', n),
        this.logs.unshift(...e));
    } finally {
      this.isSending = !1;
    }
  ...
  static startAutoFlush() {
    ...
    ((this.flushTimer = setInterval(() => {
      ...
      this.config.isProduction ||
        console.log(`[LogCollector] Auto flush started (interval: ${t}ms)`));
  }
  /**
   * 자동 전송 중지
   */
...
class S {
  ...
  static init(t = !1) {
    ...
    ((this.errorHandler = e => {
      const i = e.error?.__traceId || h.getTraceId(),
        n = {
          timestamp: /* @__PURE__ */ new Date().toISOString(),
          traceId: i,
          level: 'ERROR',
...
${e.error?.stack || ''}`,
      ...
      (this.rejectionHandler = e => {
        const i = e.reason?.__traceId || h.getTraceId(),
          n = {
            timestamp: /* @__PURE__ */ new Date().toISOString(),
            traceId: i,
            level: 'ERROR',
...
${e.reason?.stack || ''}`,
      ...
      this.isProduction || console.log('[LogLens] ErrorCapture initialized'));
  }
  static isEnabled() {
...
function T(o, t, e, s) {
  d.addLog({
    ...y(o, t, 'ERROR', e?.stack || e?.message || String(e)),
    request: {
      method: t,
    },
    response: null,
...
function R(o, t) {
  const e =
      t?.logger || (o.name && o.name !== 'anonymous' ? o.name : null) || P(),
    s = o.constructor.name === 'AsyncFunction';
  return (...i) => {
...
const v = {
  ...
  C = o => {
    ...
    const e = t.domain.trim(),
      i = `${e.endsWith('/') ? e.slice(0, -1) : e}/api/logs/frontend`;
    (t.isProduction ||
      console.log('[LogLens] Log collection enabled. Endpoint:', i),
      d.init({
        maxLogs: t.maxLogs,
        autoFlush: {
