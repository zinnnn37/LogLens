...
class TrafficMapperTest {
    ...
    @Test
    void Summary의_totalLogs가_모든_Aggregation의_합이다() {
        // given
        LocalDateTime startTime = LocalDateTime.of(2025, 11, 13, 15, 0);
        LocalDateTime endTime = startTime.plusHours(24);
        List<TrafficAggregation> aggregations = create8Aggregations();
    ...
    void 타임스탬프가_정확히_일치하지_않아도_같은_시간대로_매칭된다() {
        ...
        List<TrafficAggregation> aggregations = List.of(
                new TrafficAggregation(LocalDateTime.of(2025, 11, 13, 15, 30, 45), 1500, 800, 700),  // 15시대
                new TrafficAggregation(LocalDateTime.of(2025, 11, 13, 18, 15, 30), 1800, 950, 850)   // 18시대
        );
        ...
        assertThat(response.dataPoints().get(0).totalCount()).isEqualTo(1500);
        assertThat(response.dataPoints().get(0).feCount()).isEqualTo(800);
        assertThat(response.dataPoints().get(0).beCount()).isEqualTo(700);

        // 18:00 슬롯에 18:15:30 데이터가 매칭됨
        assertThat(response.dataPoints().get(1).totalCount()).isEqualTo(1800);
        assertThat(response.dataPoints().get(1).feCount()).isEqualTo(950);
        assertThat(response.dataPoints().get(1).beCount()).isEqualTo(850);

        // 나머지 슬롯은 0
        assertThat(response.dataPoints().get(2).totalCount()).isEqualTo(0);

        // Summary는 실제 aggregations 기반으로 계산 (dataPoints가 아님)
        assertThat(response.summary().totalLogs()).isEqualTo(3300);
        assertThat(response.summary().totalFeCount()).isEqualTo(1750);
        assertThat(response.summary().totalBeCount()).isEqualTo(1550);
        assertThat(response.summary().peakCount()).isEqualTo(1800);
    }
