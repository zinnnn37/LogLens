...
/**
 * AlertHistoryController 통합 테스트 (새 필드 alertLevel, traceId 포함)
 */
@SpringBootTest
...
class AlertHistoryControllerTest {
    ...
    class GetAlertHistoriesTest {
        ...
        @Test
        @DisplayName("GET_/api/alerts/histories_전체_조회_성공_새필드포함")
        void GET_전체_조회_성공_새필드포함() throws Exception {
            // given
            List<AlertHistoryResponse> responses = Arrays.asList(
                    createAlertHistoryResponse(1, "N", "ERROR", "trace-abc-123"),
                    createAlertHistoryResponse(2, "Y", "WARN", "trace-def-456")
            );
            ...
            mockMvc.perform(get("/api/alerts/histories")
                    ...
                    .andExpect(jsonPath("$.data[0].alertLevel").value("ERROR"))
                    .andExpect(jsonPath("$.data[0].traceId").value("trace-abc-123"))
                    .andExpect(jsonPath("$.data[1].id").value(2))
                    .andExpect(jsonPath("$.data[1].resolvedYN").value("Y"))
                    .andExpect(jsonPath("$.data[1].alertLevel").value("WARN"))
                    .andExpect(jsonPath("$.data[1].traceId").value("trace-def-456"));
        ...
        void GET_읽지_않음_필터링_성공() throws Exception {
            ...
            List<AlertHistoryResponse> responses = Collections.singletonList(
                    createAlertHistoryResponse(1, "N", "ERROR", "trace-unread-123")
            );
        ...
        @Test
        @DisplayName("GET_/api/alerts/histories_alertLevel_null_처리_성공")
        void GET_alertLevel_null_처리_성공() throws Exception {
            // given
            List<AlertHistoryResponse> responses = Collections.singletonList(
                    createAlertHistoryResponse(1, "N", null, null)
            );
            ...
            mockMvc.perform(get("/api/alerts/histories")
                    ...
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data[0].alertLevel").doesNotExist())
                    .andExpect(jsonPath("$.data[0].traceId").doesNotExist());
        }
    }
    ...
    class MarkAsReadTest {
        ...
        @Test
        @DisplayName("PATCH_/api/alerts/{alertId}/read_읽음_처리_성공_새필드포함")
        void PATCH_읽음_처리_성공_새필드포함() throws Exception {
            // given
            AlertHistoryResponse response = createAlertHistoryResponse(ALERT_ID, "Y", "ERROR", "trace-read-test");
            ...
            mockMvc.perform(patch("/api/alerts/{alertId}/read", ALERT_ID))
                    ...
                    .andExpect(jsonPath("$.data.alertLevel").value("ERROR"))
                    .andExpect(jsonPath("$.data.traceId").value("trace-read-test"));
        ...
        void PATCH_이미_읽은_알림_재요청_성공() throws Exception {
            ...
            AlertHistoryResponse response = createAlertHistoryResponse(ALERT_ID, "Y", "WARN", "trace-already-read");
            ...
            mockMvc.perform(patch("/api/alerts/{alertId}/read", ALERT_ID))
                    ...
                    .andExpect(jsonPath("$.code").value(ALERT_MARKED_AS_READ.getCode()))
                    .andExpect(jsonPath("$.data.resolvedYN").value("Y"))
                    .andExpect(jsonPath("$.data.alertLevel").value("WARN"));
    ...
    private AlertHistoryResponse createAlertHistoryResponse(Integer id, String resolvedYN, String alertLevel, String traceId) {
