  ...
  const bucketedMatrix = useMemo(() => {
    return TIME_BUCKETS.map(bucket => {
      const cells = normalizedByDay.map(day => {
        ...
        const totalError = hoursInRange.reduce((sum, h) => sum + h.errorCount, 0);
  ...
  return (
    <Card className="h-full shadow-sm">
      <CardHeader className="pb-4">
        <CardTitle className="text-lg font-bold">주간 로그 발생 히트맵</CardTitle>
      </CardHeader>
      <CardContent>
        <TooltipProvider>
          <div className="space-y-3">
            ...
            {bucketedMatrix.map(row => (
              <div key={row.label} className="flex items-center gap-3">
                <div className="w-12 text-xs font-medium text-gray-500 whitespace-nowrap">
                  {row.label.split('시')[0]}
                </div>
                  ...
                  {row.cells.map(cell => (
                    <Tooltip key={`${row.label}-${cell.dayKey}`}>
                      ...
                      <TooltipContent side="top" className="p-3 text-xs">
                        <div className="mb-2 font-bold text-sm border-b pb-1">
                          {cell.dayName}요일 {cell.bucketLabel}
                        </div>
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                          <span className="text-gray-500">전체 로그</span>
                          <span className="font-mono font-medium text-right">{cell.totalCount.toLocaleString()}</span>
                          
                          <span className="text-red-500 font-medium">Error</span>
                          <span className="font-mono text-red-600 text-right">{cell.totalError.toLocaleString()}</span>
                          
                          <span className="text-amber-500 font-medium">Warn</span>
                          <span className="font-mono text-amber-600 text-right">{cell.totalWarn.toLocaleString()}</span>
                          
                          <span className="text-blue-500 font-medium">Info</span>
                          <span className="font-mono text-blue-600 text-right">{cell.totalInfo.toLocaleString()}</span>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                  ))}
