...
def create_logs_index_template():
    """
    DEPRECATED: Log indices are now managed externally in the new format.

    New format: {project_uuid_with_underscores}_{YYYY}_{MM}
    Example: 3a73c7d4_8176_3929_b72f_d5b921daae67_2025_11

    This function is kept for reference but will not create indices.
    """
    print("ℹ️  Log indices are managed externally - skipping creation")
    print("   New format: {uuid}_{YYYY}_{MM}")
    return True  # Return success to not block application startup

    # COMMENTED OUT - Old logs-* template creation logic
    # template_body = {
    #     "index_patterns": ["logs-*"],
    #     "settings": {
    #         "number_of_shards": 1,
