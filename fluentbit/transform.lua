...
local function simple_json_parse(str)
    ...
    for key, val in string.gmatch(str, '"([^"]+)"%s*:%s*([%d%.%-]+)') do
        if tonumber(val) then obj[key] = tonumber(val) end
    end
    if next(obj) then return obj else return nil end
end
...
function transform_log(tag, timestamp, record)
    ...
    if not layer and new_record["logger"] then
        ...
        if string.match(log, "Controller") then layer = "Controller"
        elseif string.match(log, "Service") then layer = "Service"
        elseif string.match(log, "Repository") then layer = "Repository"
        elseif string.match(log, "Filter") then layer = "Filter"
        else layer = "Other" end
    end
    ...
    if lvl ~= "INFO" and lvl ~= "WARN" and lvl ~= "ERROR" then lvl = "INFO" end
    ...
    ----------------------------------------------------
    local comment_parts = {}
    if record["thread"] then table.insert(comment_parts, "thread: " .. record["thread"]) end
    if record["app_name"] then table.insert(comment_parts, "app: " .. record["app_name"]) end
    if record["pid"] then table.insert(comment_parts, "pid: " .. tostring(record["pid"])) end
    new_record["comment"] = table.concat(comment_parts, ", ")

    new_record["class_name"] = record["class_name"] or nil
    new_record["method_name"] = record["method_name"] or nil
    ...
    if type(exec_time) == "string" then exec_time = tonumber(exec_time) end
    ...
    if has_details then new_record["log_details"] = log_details end
